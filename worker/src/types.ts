...
export type Env = {
  ...
  // Optional reader/source backends. These are URLs, not API keys.
  // Nyora is keyless and active by default; the others are optional
  // self-hosted/community backends and are enabled only when configured.
  NYORA_BASE_URL?: string;
  NYORA_SOURCES?: string;
  HACHI_URL?: string;
  COMIX_API_URL?: string;
  MANGAFIRE_API_URL?: string;
  MANGATO_API_URL?: string;
  MANGA_MAPPER_URL?: string;
  MANGA_NOVEL_API_URL?: string;
  BATO_PARSER_URL?: string;
