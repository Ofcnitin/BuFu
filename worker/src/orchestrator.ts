...
import { nowIso, withDeadline, withDeadlineOrThrow } from './utils';

const PER_PROVIDER_DEADLINE_MS = 6000;
const COOLDOWN_MS = 5 * 60 * 1000;

// ── Reader-source priority ────────────────────────────────────────────
// A canonical title can now have chapters mapped from more than one
// reader-role source: MangaDex (BuFu's first-party, ToS-verified reader —
// see adapters/mangadex.ts) plus any configured Nyora-backed or self-hosted
// community adapters (adapters/nyora.ts, adapters/external.ts). Those
// aren't interchangeable in reliability or provenance, so getChapters/
// getPages always try mapped sources in this fixed order — MangaDex first,
// everything else after, in a stable order — rather than whichever mapping
// row source_mappings/source_chapter_mappings happens to return first
// (unordered SELECTs, so that was effectively arbitrary per query).
const READER_SOURCE_PRIORITY: Record<string, number> = { mangadex: 0 };
function readerPriority(sourceId: string): number {
  return READER_SOURCE_PRIORITY[sourceId] ?? 100;
}
function sortByReaderPriority<T extends { sourceId: string }>(items: T[]): T[] {
  return [...items].sort((a, b) => readerPriority(a.sourceId) - readerPriority(b.sourceId));
}
...
export async function searchAll(env: Env, q: string): Promise<ReturnType<typeof toWire>[]> {
  ...
  const results = await Promise.all(providers.map(a =>
    // withDeadlineOrThrow (not withDeadline) here: it rejects on a timeout or
    // provider error instead of resolving to a same-shaped empty array, so
    // callWithHealth's own try/catch — not this deadline helper — is what
    // decides success vs failure, and a genuine empty result is never
    // mistaken for one.
    callWithHealth(env, a.id, () => withDeadlineOrThrow(a.search!(env, q), PER_PROVIDER_DEADLINE_MS), [] as ProviderTitle[])
  ));
...
export async function recommendations(env: Env, category = 'action'): Promise<ReturnType<typeof toWire>[]> {
  ...
  // withDeadlineOrThrow, as in searchAll: lets callWithHealth's try/catch
  // tell a timed-out/errored provider apart from one that genuinely came
  // back with nothing.
  const [shiroSeeds, anilistTop, jikanTop] = await Promise.all([
    shiro?.recommendations ? callWithHealth(env, 'shiro', () => withDeadlineOrThrow(shiro.recommendations!(env, category), PER_PROVIDER_DEADLINE_MS), [] as ProviderTitle[]) : Promise.resolve([]),
    anilist?.recommendations ? callWithHealth(env, 'anilist', () => withDeadlineOrThrow(anilist.recommendations!(env, category), PER_PROVIDER_DEADLINE_MS), [] as ProviderTitle[]) : Promise.resolve([]),
    jikan?.recommendations ? callWithHealth(env, 'jikan', () => withDeadlineOrThrow(jikan.recommendations!(env, category), PER_PROVIDER_DEADLINE_MS), [] as ProviderTitle[]) : Promise.resolve([]),
  ]);
  ...
  // Canonicalized sequentially, not via Promise.all — two providers whose
  // titles normalize to the same key must never race each other's "does a
  // canonical title already exist?" lookup, or both can miss and each
  // create its own duplicate canonical title for the same work. searchAll()
  // already does this correctly; recommendations() previously didn't.
  const canonicalResults: (CanonicalTitle | null)[] = [];
  for (const p of allProviders) {
    try {
      canonicalResults.push(await canonicalizeTitle(env, p));
    } catch {
      canonicalResults.push(null);
    }
  }
...
export async function getChapters(env: Env, titleId: string): Promise<ChapterWire[]> {
  ...
  if (!title) return [];
  // Try every mapped reader source, highest priority first, until one
  // actually returns chapters — not just the first mapping row the DB
  // happens to return. A configured-but-down community adapter (circuit
  // open, timeout, error) no longer silently shadows a working MangaDex
  // mapping on the same title.
  const readerMappings = sortByReaderPriority(
    title.sourceMappings.filter(m => sourceList.find(a => a.id === m.sourceId)?.role === 'reader')
  );
  for (const readerMapping of readerMappings) {
    const adapter = sourceList.find(a => a.id === readerMapping.sourceId && a.production && a.configured(env) && a.chapters);
    if (!adapter || await isCircuitOpen(env, adapter.id)) continue;
    const start = Date.now();
    try {
      const chapters = await withDeadline(adapter.chapters!(env, readerMapping.sourceTitleId), 9000, null as any);
      // `chapters` is `null` only on withDeadline's own timeout/rejection
      // fallback — a real, successful response is always an array, and an
      // empty array (`[]`) is truthy in JS. That previously made a
      // successful-but-empty response `break` the loop exactly like a real
      // chapter list would, permanently shadowing every lower-priority
      // source on this title (e.g. a MangaDex mapping that briefly returns
      // no chapters would block a working Nyora mapping from ever being
      // tried). Distinguish the three real outcomes instead: null (record
      // failure, keep trying), a genuine empty list (record success — the
      // call worked, it just had nothing — but keep trying other mapped
      // sources rather than stopping here), and a non-empty list (the only
      // case that should actually stop the loop).
      if (chapters && chapters.length > 0) {
        await recordHealth(env, adapter.id, true, Date.now() - start);
        await persistChapters(env, titleId, chapters.map((c: any) => ({ ...c })));
        break; // got a live, non-empty chapter list from the highest-priority available source
      }
      if (chapters) {
        // Empty but genuine: the source is healthy, it just has nothing
        // for this title right now. Don't penalize its health, but don't
        // stop here either — a lower-priority source may still have
        // chapters.
        await recordHealth(env, adapter.id, true, Date.now() - start);
        continue;
      }
...
export async function getPages(env: Env, titleId: string, chapterId: string): Promise<UnifiedPage[] | null> {
  ...
  const orderedMappings = sortByReaderPriority<{ source_id: string; source_chapter_id: string; sourceId: string }>(
    (mappingRows.results || []).map((m: { source_id: string; source_chapter_id: string }) => ({
      source_id: m.source_id,
      source_chapter_id: m.source_chapter_id,
  ...
  for (const m of orderedMappings) {
    ...
    try {
      ...
      // Same non-empty-vs-empty-vs-null distinction as getChapters above:
      // `[]` is truthy, so a source that successfully responds with zero
      // pages must not short-circuit `return pages` here — that would
      // permanently return "no pages" instead of trying a lower-priority
      // mapped source that might actually have them.
