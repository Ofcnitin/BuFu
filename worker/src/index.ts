...
// Reader adapters must be registered from the application entry point.
// Do NOT import these from registry.ts — that creates a circular dependency:
// index.ts → nyora.ts → registry.ts → nyora.ts
import './adapters/nyora';
import './adapters/external';
...
import {
  ...
  getPages,
  getTitleById,
  probeAllSourceHealth,
  recommendations,
  resolveLatestChapterId,
  searchAll,
} from './orchestrator';
...
async function handle(request: Request, env: Env): Promise<Response> {
  ...
  if (path === '/api/auth/register' && method === 'POST') {
    ...
    return withCors(
      request,
      env,
      await register(env, await body(request)),
    );
  }
  ...
  if (path === '/api/auth/login' && method === 'POST') {
    ...
    return withCors(
      request,
      env,
      await login(env, await body(request)),
    );
  }
  ...
  const isDiscoveryRoute =
    ...
    /^\/api\/title\/[^/]+(\/chapters|\/chapter\/[^/]+\/pages)?$/.test(
      path,
    );
  ...
  if (
    ...
    const category = (
      url.searchParams.get('category') || 'action'
    )
      .trim()
      .slice(0, 40);
  ...
  if (
    ...
    const rows = await env.DB.prepare(
      `SELECT source_id,ok,consecutive_failures,
              circuit_open_until,checked_at
       FROM source_health`,
    ).all();
  ...
  // ── Titles ──────────────────────────────────────────────────────────
  const pagesMatch = path.match(
    /^\/api\/title\/([^/]+)\/chapter\/([^/]+)\/pages$/,
  );

  if (pagesMatch && method === 'GET') {
    const titleId = decodeURIComponent(
      pagesMatch[1],
    );

    let chapterId = decodeURIComponent(
      pagesMatch[2],
    );
    ...
    if (pages === null) {
      return withCors(
        ...
        json(
          {
            error:
              'Chapter not found or no source could serve it',
          },
          { status: 404 },
        ),
      );
  ...
  const chaptersMatch = path.match(
    /^\/api\/title\/([^/]+)\/chapters$/,
  );
  ...
  if (path === '/api/me/library') {
    ...
    if (method === 'GET') {
      const rows = await env.DB.prepare(
        `SELECT l.title_id,l.status,
                l.last_seen_chapter_number,
                l.added_at,l.updated_at,
                t.title,t.cover_url,t.type,
                t.status as title_status,
                t.latest_known_chapter_number
         FROM library l
    ...
    if (method === 'PUT') {
      ...
      await env.DB.prepare(
        `INSERT INTO library(
           user_id,title_id,status,added_at,updated_at
         )
         VALUES(?,?,?,?,?)
    ...
    if (method === 'DELETE') {
      ...
      await env.DB.prepare(
        'DELETE FROM library WHERE user_id=? AND title_id=?',
      )
  ...
  if (path === '/api/me/bookmarks') {
    ...
    if (method === 'GET') {
      const rows = await env.DB.prepare(
        `SELECT b.title_id,b.created_at,
                t.title,t.cover_url,t.type
         FROM bookmarks b
    ...
    if (method === 'PUT') {
      ...
      if (b.remove) {
        await env.DB.prepare(
          'DELETE FROM bookmarks WHERE user_id=? AND title_id=?',
        )
          .bind(user!.id, titleId)
          .run();
      } else {
  ...
  if (path === '/api/me/history') {
    ...
    if (method === 'GET') {
      const rows = await env.DB.prepare(
        `SELECT h.title_id,h.chapter_id,h.updated_at,
                t.title,t.cover_url,t.type
         FROM history h
    ...
    if (method === 'POST') {
      ...
      let historyChapterId: string | null =
        null;
        ...
        if (!chapterOk) {
          return withCors(
            ...
            json(
              {
                error:
                  'chapterId does not belong to titleId',
              },
              { status: 400 },
            ),
          );
      ...
      await env.DB.prepare(
        `INSERT INTO history(
           user_id,title_id,chapter_id,updated_at
         )
         VALUES(?,?,?,?)
         ON CONFLICT(user_id,title_id)
         DO UPDATE SET
  ...
  if (path === '/api/me/progress') {
    ...
    if (method === 'PUT') {
      ...
      if (!titleId || !chapterId) {
        return withCors(
          ...
          json(
            {
              error:
                'titleId and chapterId required',
            },
            { status: 400 },
          ),
        );
      ...
      const chapterRow =
        await env.DB.prepare(
          `SELECT c.id,
                  MAX(m.pages_count) as pages_count
           FROM canonical_chapters c
           LEFT JOIN source_chapter_mappings m
             ON m.chapter_id=c.id
           WHERE c.id=? AND c.title_id=?
           GROUP BY c.id`,
        )
          .bind(chapterId, titleId)
          .first<{
            id: string;
      ...
      let pageIndex = Math.max(
        0,
        Math.floor(pageIndexInput),
      );

      const knownPageCount =
        chapterRow.pages_count;
      ...
      const scrollRatio = Math.min(
        ...
        Math.max(
          0,
          Number.isFinite(rawScrollRatio)
            ? rawScrollRatio
            : 0,
        ),
      );
      ...
      const clientUpdatedAt =
        typeof b.clientUpdatedAt === 'string' &&
        !isNaN(
          Date.parse(b.clientUpdatedAt),
        )
          ? b.clientUpdatedAt
          : null;
      ...
      await env.DB.prepare(
        `INSERT INTO reading_progress(
           ...
           chapter_number,page_index,
           scroll_ratio,completed,updated_at
         )
         VALUES(?,?,?,?,?,?,?,?)
         ON CONFLICT(user_id,title_id)
         DO UPDATE SET
  ...
  if (
    ...
    const rows = await env.DB.prepare(
      `SELECT l.title_id,
       ...
       JOIN titles t ON t.id=l.title_id
       WHERE l.user_id=?
         AND t.latest_known_chapter_number IS NOT NULL
         AND (
           l.last_seen_chapter_number IS NULL
           OR t.latest_known_chapter_number >
              l.last_seen_chapter_number
         )
       ORDER BY
         t.latest_known_chapter_number DESC`,
    )
      .bind(user!.id)
      .all();
...
async function refreshStaleLibraryTitles(
  env: Env,
): Promise<void> {
  const rows = await env.DB.prepare(
    `SELECT DISTINCT t.id
     ...
     WHERE EXISTS (
       ...
       WHERE m.title_id=t.id
         AND m.source_id='mangadex'
     )
     ORDER BY t.updated_at ASC
     LIMIT ?`,
  )
    .bind(STALE_LIBRARY_REFRESH_BATCH)
    .all<{ id: string }>();
  ...
  for (const row of rows.results || []) {
    try {
      ...
      // Best-effort: one title's failure
