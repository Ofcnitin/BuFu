...
async function handle(request: Request, env: Env): Promise<Response> {
  ...
  // ── Discovery ───────────────────────────────────────────────────────
  // These routes are intentionally public (no requireUser gate — browsing
  // is meant to work signed-out), but each one fans out to several
  // third-party providers per call, so unlike a simple static route they're
  // a real cost/abuse amplification target: cheap for a caller, expensive
  // against both this Worker's own budget and every upstream provider's
  // shared-per-isolate rate limit. Same IP-bucketed guard as auth, just a
  // much higher ceiling since these are meant to be hit on every page load.
  const isDiscoveryRoute =
    (path === '/api/search' || path === '/api/home' || path === '/api/recommendations') ||
    /^\/api\/title\/[^/]+(\/chapters|\/chapter\/[^/]+\/pages)?$/.test(path);
  if (isDiscoveryRoute && method === 'GET' && !(await checkRateLimit(env, 'discovery', clientIp(request)))) {
    return withCors(request, env, json({ error: 'Too many requests. Try again shortly.' }, { status: 429 }));
  }
