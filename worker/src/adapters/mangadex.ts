...
const adapter: SourceAdapter = {
  ...
  async search(env, q) {
    ...
    // BUFU has no age gate or adult-content toggle anywhere in the app, so
    // content rating is filtered here explicitly rather than left to
    // whatever MangaDex's unstated default is — safe/suggestive only.
    // Never widen this without an actual age-verification flow.
  ...
  async chapters(env, mangaId) {
    ...
    for (;;) {
      ...
      // Was previously opted in to 'erotica'/'pornographic' as well — with
      // no age gate anywhere in BUFU, that meant any MangaDex-mapped title
      // could serve explicit chapters to any anonymous visitor. Matches the
      // safe/suggestive-only filter now used in search()/getTitle() above.
      u.searchParams.append('contentRating[]', 'safe');
      u.searchParams.append('contentRating[]', 'suggestive');
      const r = await fetchJson(u.toString(), { headers }, 8000, 'mangadex');
