 ...
 *  - nyora: no documented public rate limit found; self-imposed conservative
 *    cap. Deliberately keyed as ONE shared bucket ('nyora') across every
 *    Nyora-backed source (bato, comick, mangafreak, ...) rather than one
 *    bucket per source id — they all resolve to the same origin
 *    (NYORA_BASE_URL), and a per-source key would let a single search fan
 *    out N times the intended request rate against that one shared host.
 */
