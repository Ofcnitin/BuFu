...
function mapSeed(x: any): ProviderTitle | null {
  ...
  return {
    ...
    title: name,
    altTitles: [],
    // Unresolved — this is just a title seed from SHIRO's recommendation
    // feed, not a verified classification. resolveSeed() below replaces
    // this whole object with mapMangaDex()'s independently-derived type
    // whenever a real MangaDex match is found; type/readingMode here only
    // survive as-is when that match fails, and 'unknown'/'auto' is the
    // honest state for "we don't actually know yet" rather than guessing
    // manhwa/vertical from the endpoint name alone.
    type: 'unknown',
